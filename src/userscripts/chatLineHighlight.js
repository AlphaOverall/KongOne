...
class ChatLineHighlight extends HolodeckScript {
    constructor() {
        super('Chat Line Highlight', /^\/games/, true, Script.CATEGORIES.CHAT);
    }
    ...
    run() {
        ...
        if (CRoom && CDialogue) {
            ...
            if (!CDialogue.prototype.searchWord) {
                ...
                let generateCallback = (name, stop, start) => {
                    return function(l, n) {
                        if (l["_hl_" + name]) {
                            l["_hl_" + name] = 0;
                ...
                let generateColorCallback = (selector, rule, name, text, max) => {
                    if (!max) max = 1;
                    return function(l, n) {
                        ...
                        if (k) z = "#" + k[1];
                        if (z) {
                            let f = () => GM_setValue("kong_" + name, z);
